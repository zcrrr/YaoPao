...
- (void)downloadRecordDidSuccess:(NSDictionary *)resultDic{
    ...
    for(int i=0;i<[records count];i++){
        ...
        if(![runClass.serverBinaryFilePath isEqualToString:@""]){
            [self.fileArray addObject:[NSString stringWithFormat:@"%i,%@,%@,3",i,runClass.rid,runClass.serverBinaryFilePath]];
        }
        if(![runClass.serverImagePaths isEqualToString:@""]){
            [self.fileArray addObject:[NSString stringWithFormat:@"%i,%@,%@,21",i,runClass.rid,runClass.serverImagePaths]];
        }
        if(![runClass.serverImagePathsSmall isEqualToString:@""]){
            [self.fileArray addObject:[NSString stringWithFormat:@"%i,%@,%@,22",i,runClass.rid,runClass.serverImagePathsSmall]];
        }
    }
    self.fileCount = [self.fileArray count];
    [self downloadfile];
